...
| 4 | LL-006 / LL-008 / LL-018 | `P_detect ≥ 1 - K·exp(-c·T·δ_A²)` isotropic, with the per-class A1..A6 quantification. **Composition foothold 0.0.54** (`LL006_worst_case_lower_than_isotropic`): worst-case bound value `≤` isotropic bound value, via `LL021_eff_squared_bound` + `Real.exp` monotonicity + `linarith`. Bound-shape monotonicity proved; LL-006 stays `:benchmarked` — the full `P(detect) ≥ ...` lower bound is a probability-space formalization (multi-session). | Mathlib Real + Mathlib Analysis (used) |
